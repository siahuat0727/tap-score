...
/// Editing state and commands for the score editor.
class EditorController extends ChangeNotifier {
  ...
  void handleRestAction() {
    if (_selectionKind == SelectionKind.note && _selectedNoteIndex != null) {
      ...
      if (!updated.isRest) {
        _notePreview.previewNote(updated.midi);
      }
  ...
  void selectNote(int? index) {
    ...
    if (index != null) {
      ...
      final note = score.notes[index];
      if (!note.isRest) {
        _notePreview.previewNote(note.midi);
      }
    } else {
      _selectionKind = null;
      _selectedNoteIndex = null;
  ...
  void _insertNotesAtCursor(List<Note> notes) {
    ...
    if (soundedNote != null) {
      _notePreview.previewNote(soundedNote.midi);
    }
  }
  ...
  void _diatonicStepSelected(int direction) {
    ...
    _notePreview.previewNote(newMidi);
    _markScoreChanged();
  }
